...
>>>>>>> 91a53b6d9f8ae0a14291ad3567eac56a8dad6f1c
    ...
    public function export_pdf_seguro($id)
    {
        $user = User::query()
            ->with(['membresia'])
            ->where('id', $id)
        ...
        $consecutivo = consecutivos::where('nombre', 'Consecutivo_seguro')->first()->valor;
        $consecutivo = $consecutivo + 1;
        consecutivos::where('nombre', 'Consecutivo_seguro')->update([
            'valor' => $consecutivo
        ]);
        ...
        User::query()
            ->where('id', $id)
            ->update([
                "json_consecutivos" => $array_consecutivos
            ]);
        $customPaper = array(0, 0, 1920, 720);
        $customPaper = 'letter';
        $pdf = \PDF::loadView('correo_seguro', [
            'user' => $user,
    ...
    public function index(Request $request)
    {
        ...
        if ($name) {
            ...
            $query->orWhere('json_consecutivos', 'like', '%' . $name . '%');
        }
        if ($active) {
